...
Advent of Code 2022 solutions in Rust. Days 1–24 implemented.
...
`<day>` is `1`–`24`, `<part>` is `1` or `2`. `[extra]` overrides the default for the puzzles that need it:
...
Runs unit tests (parser correctness, malformed-input errors, `Signal` ordering) plus 24 integration tests that exercise every `partN` against its sample input.
